...
At \(n=1{,}000{,}000\), only bubble sort produced results. After it finished, the program crashed with a segmentation fault (exit code 139). The cause is a \textbf{stack memory overflow} in the recursive quick sort implementation. Here is why:
...
\begin{itemize}
    ...
    \item With the last-element pivot strategy, a reversed or sorted input always creates the worst-case split: one sub-array of size 0 and one of size \(n-1\). This means the recursion goes \(n\) levels deep instead of the expected \(O(\log n)\).
...
Bubble sort is iterative (no recursion) so it was not affected and completed normally. Merge sort is also recursive, but since it always splits the array in half, its recursion depth is only \(O(\log_2 n) \approx 20\) levels at \(n = 1{,}000{,}000\), which is safe.
